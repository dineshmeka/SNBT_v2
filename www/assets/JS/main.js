$(document).ready(function () {
    // Determine the path to Nav.html based on current directory
    var path = window.location.pathname;
    var navPath = "Nav.html"; // Default for root

    const isSubfolder = path.includes('/About/') ||
        path.includes('/Gallery/') ||
        path.includes('/Admissions/') ||
        path.includes('/Admin-staff/') ||
        path.includes('/Contact/') ||
        path.includes('/Results/') ||
        path.includes('/Student-services/') ||
...
// the below script is used for the slideshow of about snbt
document.addEventListener('DOMContentLoaded', function () {
    // Call showSlides function after navigation bar is loaded
    var aboutSnbtDiv = document.getElementById("aboutsnbt");
    if (aboutSnbtDiv) {
        let slideIndex = 0;
        showSlides();
    ...
    if (galleryDiv) {
        ...
        gallery.querySelectorAll('img').forEach(function (item) {
            ...
            if (item.complete) {
                ...
                item.addEventListener('load', resizeItem);
            }
        });
